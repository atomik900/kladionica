...
#  dok ne uradim serverski deo koji ce slati kvote uzivo sa servera, moram da instanciram sve ovo ispod 
...
class User:
    ...
    def show_matches(self):
        ...
        # OVO CE ICI KAD SE NAPISE SERVERSKA APLIKACIJA KOJA SALJE UZIVO KVOTE
        # klijent = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
        # klijent.connect(('localhost',8888))
        # lista_opklada = klijent.recv(1024)
        # while lista_opklada:
            #print('recieving available matches')
            #lista_opklada = klijent.recv(1024)
    ...
    def proceed_to_payment(self):
        ...
        if amount_to_bet > self.credit:
            ...
            #NESTO OVDE NE FUNKCIONISE , VRACA PREVELIKE KVOTE
            ...
            if definitive == '1':
                db = conn.connect(
                    ...
                    database="kladionica",
                    passwd="pozz"
                    # napravio sam usera na mysql serveru i dodao grantove za parametre sa ip adrese mog kucnog kompa
                )
                cur = db.cursor()
...
name = input('Please enter your name to enter SPORTS BETS 0-24\n')
money = int(input('Please enter the ammount of money you wish to spend\n'))
print(f' WELCOME {name} !')
user1 = User(name,money)
user1.start_gambling()



# print(sys.getrefcount(bilbao))
